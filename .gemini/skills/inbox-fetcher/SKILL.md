...
This skill is a **pre-ingest step**. After it runs, the user (or the agent following the vault's `GEMINI.md`) performs the actual ingest — reading the new files in `raw/` and compiling them into the wiki.
...
```bash
python .gemini/skills/inbox-fetcher/scripts/fetch_inbox.py
```

Or from anywhere:

```bash
python .gemini/skills/inbox-fetcher/scripts/fetch_inbox.py --vault /path/to/vault
```
